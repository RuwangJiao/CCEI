function [alpha, Kf, L, K, Kxstar, Kss] = alpha_mtgp(logtheta, covfunc_x, x, y, m, irank, ...
    ...
    try
        ...
        K = K + diag(diag(ones(size(K,1),size(K,1))*1e-10));
        try
            L = chol(K)';
        catch
            K = K + diag(diag(ones(size(K,1),size(K,1))*1e-8));
            L = chol(K)';
        end
    end
